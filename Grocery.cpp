...
void Grocery::loadItemsFromFile(const string &filename)
{
    ...
    while (inFile >> item)
    {
        ...
        /*uses the map by applying the key item then increasing the value + 1 every time the
        item name pops up*/
        itemFrequency[item]++;
    }
    inFile.close();
...
void Grocery::searchForItem(const string &item)
{
    ...
    {   // it -> accesses the value of the iterator while it -> second gets the frequency 
